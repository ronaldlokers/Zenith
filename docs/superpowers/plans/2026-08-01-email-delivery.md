...
describe("resolveProvider", () => {
  ...
  it("returns the resend provider when a key is present", () => {
    const p = resolveProvider({ ...env, RESEND_API_KEY: "re_test" });
...
describe("sendEmail", () => {
  it("does not send, and does not throw, when no key is configured", async () => {
    // No interceptor registered: any outbound request would fail the run.
    await expect(sendEmail({ ...env, RESEND_API_KEY: undefined }, MSG)).resolves.toBe(false);
  });

  it("posts the message to resend and disables tracking", async () => {
    ...
    const sent = await sendEmail({ ...env, RESEND_API_KEY: "re_test" }, MSG);
    ...
    // No telemetry, ever — a tracking pixel would contradict the product's
    // first principle, and Resend enables tracking unless told otherwise.
    expect(seen).toMatchObject({ tags: undefined });
  });
  ...
  it("reports failure rather than throwing when the provider rejects", async () => {
    fetchMock
      ...
      .reply(401, { message: "API key is invalid" });
