...
- **Frontend:** React 19 + Vite + TypeScript. `src/App.tsx` is the app-shell orchestrator (routing, top-level state wiring); tab bodies live in their own feature files (`dashboard`, `board`, `feed`, `network`, `cv`, `settings`, …), shell chrome (Sidebar/TopBar/MobileTabs/ToastStack) in `src/shell.tsx`, and the app-level controller hooks in `src/app-data.ts` (`useAppData`/`useToasts`) + `src/hooks.ts` (chrome effects). The shell chrome stays **plain** (keeps its App.css classes) because `.app` is a layout grid its pieces participate in — not extractable as self-contained `zui-` components. `src/App.css` is one file in documented bands (see its header comment) — new CSS goes in bands 1–3, never after the control-normalization layer. `src/index.css` holds the design tokens.
