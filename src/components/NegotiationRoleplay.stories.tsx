...
const meta: Meta<typeof NegotiationRoleplay> = {
  title: "AI/NegotiationRoleplay",
  component: NegotiationRoleplay,
  tags: ["autodocs"],
};
export default meta;

type Story = StoryObj<typeof NegotiationRoleplay>;
...
export const InProgress: Story = {
  ...
  play: async ({ canvasElement }) => {
    const replies = [
      "We'd like to open at $165k base plus a standard equity grant. How does that sound?",
      "I can flex a little on the signing bonus — let's talk numbers.",
    ];
    let call = 0;
    ...
    try {
      ...
      await user.click(canvas.getByRole("button", { name: /start negotiation/i }));
      await canvas.findByText(replies[0]);

      await user.type(
        canvas.getByPlaceholderText(/type your reply/i),
        "Thanks for the offer — given the scope of this role I was expecting closer to $180k base.",
      );
